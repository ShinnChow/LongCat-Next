...
# Multi-node UNIFIED (mixed understanding + generation) SFT demo.
#
...
#   2. Prepare BOTH datasets once (see prepare_st_vqa.py / prepare_pokemon.py).
#      The checkpoint, both datasets, and MERGED_DATA_DIR must live on a SHARED
#      filesystem visible to every node at the SAME absolute path, and every node
#      must launch from the same repo root.
#
# How to run:
...
#   host (MASTER_ADDR must point to it and be reachable from all nodes). Node 0
#   merges + shuffles the two datasets once, before training starts.
#
#   # on node 0 (the master):
#   NNODES=4 NODE_RANK=0 MASTER_ADDR=10.0.0.1 MASTER_PORT=29500 \
#       bash examples/train_unify_multinode.sh
...
# rank 0 merges + shuffles the inputs into one JSONL here. Must be on the shared
# filesystem and writable, since all nodes read the merged file.
